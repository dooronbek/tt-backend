...
app.get('/orders', async (req, res) => {
            try {
                          ...
                          const orders = await odoo.call('sale.order', 'search_read', [domain], {
                                          fields: ['id', 'name', 'state', 'partner_id', 'amount_total', 'date_order', 'note', 'invoice_status', 'picking_note'],
                          ...
                          const result = orders.map(o => ({
                                          ...
                                          delivered: o.picking_note === 'DELIVERED' || o.state === 'done',
...
// POST /order — create + auto-confirm
app.post('/order', async (req, res) => {
            const { partnerId, leadId, lines, payMethod, delivery } = req.body;
...
app.post('/order/:id/deliver', async (req, res) => {
            ...
            try {
                          await odoo.call('sale.order', 'write', [[orderId], { picking_note: 'DELIVERED' }]);
                          res.json({ ok: true, orderId, delivered: true });
            } catch (err) { res.status(500).json({ error: err.message }); }
});

// POST /order/:id/pay — create invoice + validate
app.post('/order/:id/pay', async (req, res) => {
            const orderId = parseInt(req.params.id);
            try {
                          const result = await odoo.call('sale.order', 'action_invoice_create', [[orderId]]);
                          const invoiceId = Array.isArray(result) ? result[0] : result;
                          if (invoiceId) await odoo.call('account.move', 'action_post', [[invoiceId]]);
                          res.json({ ok: true, orderId, invoiceId });
            } catch (err) {
...
// POST /order/:id/confirm — confirm quotation (kept for reference)
