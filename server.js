...
// Helper: safely read Many2one field [id, name]
...
app.post('/submit', upload.single('photo'), async (req, res) => {
      ...
      if (!name || !lat || !lng || !tag)
              return res.status(400).json({ error: 'Required: name, lat, lng, tag' });
      try {
...
app.patch('/partner/:id/vat', async (req, res) => {
      const { vat } = req.body;
      if (vat === undefined) return res.status(400).json({ error: 'vat field required' });
      try {
      ...
      try {
              ...
              if (orderIds.length) {
                        ...
                        for (const l of lines) {
                                    ...
                                    lineMap[oid].push({
                                                  name: productF ? productF[1] : (l.name || '—'),
