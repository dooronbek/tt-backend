...
app.post('/order/:id/deliver', async (req, res) => {
    try {
          await odoo.call('sale.order', 'action_done', [[parseInt(req.params.id)]]);
          res.json({ ok: true });
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/order/:id/pay', async (req, res) => {
    try {
          const invoiceIds = await odoo.call('sale.order', '_create_invoices', [[parseInt(req.params.id)]]);
          const ids = Array.isArray(invoiceIds) ? invoiceIds : [invoiceIds];
          for (const invId of ids) await odoo.call('account.move', 'action_post', [[invId]]);
          res.json({ ok: true, invoiceIds: ids });
    } catch (err) { res.status(500).json({ error: err.message }); }
});
