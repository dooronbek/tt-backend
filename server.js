...
// GET /orders?userId=N — fetch all sales orders for agent's partners
app.get('/orders', async (req, res) => {
          try {
            ...
            const orders = await odoo.call('sale.order', 'search_read', [domain], {
                          fields: ['id', 'name', 'state', 'partner_id', 'amount_total', 'date_order', 'note'],
...
// POST /order — create new sales order
...
// POST /order/:id/confirm — confirm quotation → sales order
...
// POST /order/:id/invoice — create invoice and mark as paid
app.post('/order/:id/invoice', async (req, res) => {
          const orderId = parseInt(req.params.id);
          try {
